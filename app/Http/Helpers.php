...
if (!function_exists('filter_shops')) {
    ...
    {
        return $shop_query->where('published', 1)->where('approval', 1)->where('verification_status', 1);
    }
}
    ...
    function calculate_seller_commision($order)
    {
        ...
        if ($order->commission_calculated == 0 && $shop->user->user_type != 'admin') {
            ...
            if ($order->payment_type == 'cash_on_delivery') {
                ...
                if (get_setting('order_manage_by_admin') == 1) {
...
if (!function_exists('set_cookie')) {
    ...
    {
        $cookie = cookie('example_cookie', 'cookie_value', 60); // 60 minutes
        return response('Cookie has been set')->cookie($cookie);
        // return response('Cookie has been set')->cookie($name, $value, $time);
    }
}
